...
class render_structureTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mrbobby.plugins.PLUGINS['render_filename'] = mrbobby.plugins.load_plugin('render_filename')
...
class render_templateTest(unittest.TestCase):
    ...
    def test_render_statement_template(self):
        """if the source is not a template, it is copied."""
        filename = 'renamedtemplate2/+author.name++__if_rdr.me__+_endpoint.py.bobby'
        fs_source = os.path.join(self.fs_templates, filename)
        self.call_FUT(fs_source, {'rdr.me': 'y', 'author.name': 'bobby'},)
        self.assertTrue(os.path.exists('%s/%s' % (self.fs_tempdir, 'bobby_endpoint.py')))
    ...
    def test_render_template(self):
        ...
        fs_source = os.path.join(self.fs_templates,
            'unbound/+__if_rdr.me__+usr/local/etc/unbound/unbound.conf.bobby')
        fs_rendered = self.call_FUT(
            fs_source,
            {'ip_addr': '192.168.0.1', 'access_control': '10.0.1.0/16 allow',
              'rdr.me': 'y'})
